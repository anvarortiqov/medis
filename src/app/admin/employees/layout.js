...
const navLink = [
    { title: "Rahbariyat" , path: "management" },
    { title: "Shifokorlar", path: "doctors" },
    { title: "Texnik Xodimlar", path: "tech-workers" },
    { title: "Xodim Qo'shish", path: "add-employee" },
];
    ...
    return (
        <div className="flex flex-col justify-between">
            <header className="w-full px-4 h-14 flex justify-center items-end">
                <div className="w-full px-[0.55rem] py-[0.55rem] bg-white shadow rounded-md grid grid-cols-4 gap-5 justify-between">
                    {navLink.map((link) => (
                        <Link
                            key={link.path}
                            href={link.path}
